...
function _precompile_()
    ...
    let fbody = try __lookup_kwbody__(which(calibrate_tomogram, (CTScanner{FBP{FanBeamGeometry{Float64,DefaultTomograph},Marta.RadonAlgorithm.Filters.RamLak},Array{Float64,2}},GrayScaleLine{Float64},))) catch missing end
        if !ismissing(fbody)
            precompile(fbody, (Base.Iterators.Pairs{Union{},Union{},Tuple{},NamedTuple{(),Tuple{}}},typeof(calibrate_tomogram),CTScanner{FBP{FanBeamGeometry{Float64,DefaultTomograph},Marta.RadonAlgorithm.Filters.RamLak},Array{Float64,2}},GrayScaleLine{Float64},))
        end
    end
end
