...
function _precompile_()
    ...
    let fbody = try __lookup_kwbody__(which(FBPScanner, (ParallelBeamGeometry{Float32, DefaultTomograph},CircleImage{Float32},))) catch missing end
        if !ismissing(fbody)
            precompile(fbody, (Nothing,Nothing,Type{FBPScanner{G, D} where {G, D}},ParallelBeamGeometry{Float32, DefaultTomograph},CircleImage{Float32},))
        end
    end
